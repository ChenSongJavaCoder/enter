...
public class TestController implements TestApi {
	...
	@Override
	public String test() {
        int record = userMapper.insertSelective(new User().setAge(26).setUsername("cs_" + LocalDateTime.now().toString()).setPassword(passwordEncoder.encode(defaultPassword)));
		if (1 == record) {
	...
	public String testLogin() {
        User user = new User();
        user.setId(3L);
        User a = userMapper.selectOne(user);
	...
	public UserDto getUserByUsername(String name) {
        User user = null;
		try {
            user = userMapper.selectOne(new User().setUsername(name));
		} catch (Exception e) {
